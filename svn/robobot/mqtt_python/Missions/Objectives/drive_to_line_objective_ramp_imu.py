...
class DriveToLineObjectiveIMU(Objective):
    ...
    def tick(self, ctx):
        ...
        if self.state == DriveToLineStateIMU.START:
            ...
            if pitch > self.ramp_pitch_threshold:
                self.on_ramp = True
                print(f"Detected ramp up, pitch: {pitch:.3f} rad")

            if self.on_ramp and pitch < self.ramp_pitch_threshold:
            ...
            if self._line_lost(ctx):
                if self.on_ramp:
                    # On ramp, continue driving straight
                    ctx.actions.drive.rc(self.follow_speed, 0.0)
                    print("Line lost on ramp, continuing straight")
                else:
                    # Lost the line - stop
                    ctx.actions.edge.stop_following()
                    ctx.actions.drive.stop(instant=self.instant_stop)
